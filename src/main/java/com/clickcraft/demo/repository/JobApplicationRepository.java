...
public interface JobApplicationRepository extends JpaRepository<JobApplication, Long> {
    ...
    @Query("SELECT ja FROM JobApplication ja " + "JOIN ja.clientJobPosting jp " + "WHERE jp.clientProfile = :clientProfile")
    List<JobApplication> findClientJobApplications(ClientProfile clientProfile);

    @Query("SELECT ja FROM JobApplication ja " + "JOIN ja.clientJobPosting jp " + "WHERE jp.id = :jobId")
